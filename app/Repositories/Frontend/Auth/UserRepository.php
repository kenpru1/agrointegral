...
class UserRepository extends BaseRepository
{
    ...
    public function update($id, array $input, $image = false)
    {
        ...
        $user->rut_dni = $input['rut_dni'];
        $user->direccion = $input['direccion'];
        $user->pais_id = $input['pais_id'];
        $user->comuna_id = $input['comuna_id'];

        // Upload profile image if necessary
        if ($image) {
            $user->avatar_location = $image->store('/app/public/avatars');
            \File::delete(auth()->user()->avatar_location);
        } else {
            ...
            if ($input['avatar_type'] == 'storage') {
                ...
                if (strlen(auth()->user()->avatar_location)) {
                    //Storage::disk('local')->delete(auth()->user()->avatar_location);
                    \File::delete(auth()->user()->avatar_location);
                }
