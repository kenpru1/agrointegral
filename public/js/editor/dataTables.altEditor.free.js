    ...
    $.extend(
        altEditor.prototype,
        {
            ...
            _setup: function () {
                ...
                this.language.edit = { title: this.language.edit.title || 'Editar registro',
                ...
                this.language.success = this.language.success || 'Terminado!';
                this.language.error = this.language.error || {};
                this.language.error = { message: this.language.error.message || 'Ha ocurrido un error desconocido!',
                                        label: this.language.error.label || 'Error!',
                                        responseCode: this.language.error.responseCode || 'Mensaje respuesta : ',
                                        required: this.language.error.required || 'Campo es requerido',
            ...
            */
            createDialog: function(columnDefs, title, buttonCaption, closeCaption, buttonClass, formName) {
                ...
                var url = URLdomain + "/admin/guardarData/";
                $(selector).on('show.bs.modal', function () {
                    ...
                    if (modalContent.parent().is('form')) {
                        ...
                        modalContent.wrap("<form name='" + formName + "'method='post' action='" + url + "' id='" + formName + "' role='form'></form>");
                    }
                });
                ...
                $(document).on("click", "#"+ buttonClass, function () {
                    ...
                    $.ajax({
                        'url': "/admin/guardarData",
                        'headers': {'X-CSRF-TOKEN': $("#token").val()},
                        'type': "POST",
                        'data': {requerimiento: $("#requerimiento").val(), fecha_muestreo: $("#fecha_muestreo").val(), numero_muestra: $("#numero_muestra").val(), informe_recibido: $("#informe_recibido").val(), factura_laboratorio: $("#factura_laboratorio").val(), servicio_facturado: $("#servicio_facturado").val()},
