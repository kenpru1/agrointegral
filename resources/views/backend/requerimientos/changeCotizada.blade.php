...
@section('title', 'Requerimientos' . ' | ' . 'Cambiar Etapa Cotizada')

@section('content')
    {{ html()->modelForm($requerimiento, 'GET', route('admin.requerimientos.asociarCotizacion', $requerimiento))->class('form-horizontal')->attribute('enctype', 'multipart/form-data')->open() }}
        <div>
            <div class="ibox float-e-margins">
                <div class="ibox-title">
                    <h5>Administración de Requerimientos
                        <small class="text-muted">Cambiar Etapa Cotizada</small>
                    </h5>                               
                </div>
                <div class="ibox-content">
                    <form class="form-horizontal">
                        ...
                        <div class="row">
                            <div class="form-group col-md-5">
                                {{ html()->label('Seleccionar Coización')
                                    ->for('presupuesto_id') }}
                                @if($requerimiento->presupuesto_id != null)
                                    {{ html()->select('presupuesto_id', $presupuestos, null)
                                ...
                                @else
                                    {{ html()->select('presupuesto_id', $presupuestos, null)
                                    ->placeholder('Seleccione Nro. Cotización', null)
                                    ->class('form-control  chosen-select')
                                    ->required()
                                    ->id('presupuesto_id')
                                     }}
                            ...
                            <div class="form-group col-md-5 col-md-push-1">
                                <a href="{{ route('admin.presupuestos.create') }}" class="pull-center btn btn-xs btn-blue">Crear Cotización</a>  
                            </div>
                        </div>                   
