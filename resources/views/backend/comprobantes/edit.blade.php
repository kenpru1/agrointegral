...
@section('title', 'Editar Comprobante de Pago')

@section('content')
{{ html()->modelForm($comprobante, 'PATCH', route('admin.comprobantes.update', $comprobante))->class('form-horizontal')->attribute('enctype', 'multipart/form-data')->open() }}
    <div>
        <div class="ibox float-e-margins">
            <div class="ibox-title">
                <h5>Editar Comprobante de Pago | Nro. :
                    <b> {{ $comprobante->numero }}</b>
                </h5>
                ...
                <form class="form-horizontal">
                    <div class="row">
                        <div class="form-group col-md-5">
                            ...
                            <input type="date" class="form-control"  id="fecha" name="fecha_pago" value={{ $comprobante->fecha_pago }} required>   
                        </div>
                                ...
                                @foreach($comprobante->detalle_comprobante as $detalle)
                                    ...
                                    <tr lineid="{{ $lineId }}">
                                        <td>
                                            ...
                                            <input type="hidden" value="{{ $detalle->trabajo_realizado }}" name="trabajo_realizado_array[]">
                                        </td>
                                        <td class="text-right">
                                            <b>$</b> {{ $detalle->total }}
                                            <input type="hidden" class="sum_trabajos" value="{{ $detalle->total }}" name="total_array[]">
                                        </td>
                                        <td> 
                                            <a href="#" class="borrar_trabajos btn btn-success" data-toggle="tooltip" lineid="{{ $lineId }}" title="Eliminar"><i class="fa fa-minus"></i></a>
                                        </td>
                                    </tr>    
                                @endforeach
                                <input type="hidden" value="{{ $numero_trabajos }}" name="numero_trabajos" id="numero_trabajos">
                            </tbody>
                        </table>
                    </div>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="col-md-10 col-sm-10 col-xs-12">
                                            <div class="input-group m-b">
                                                <span class="input-group-addon">$</span> 
                                                <input type="number" class="form-control" id="total_comprobante" name="total_comprobante" value="{{ number_format($comprobante->total, 2, '.', '') }}" readonly>
                                            </div>
                                        </div>
                                    </td>
                                </tr>
...
@section('scripts')
<script>
    var numero_trabajos = $('#numero_trabajos').val();
