...
@section('content')
{{ html()->modelForm($actividadGasto, 'PATCH', route('admin.actividades_campos.update_gasto',$actividadGasto))->class('form-horizontal')->open() }}
<div id="gasto">
    {{ config('porcIva') }}
            ...
            <form class="form-horizontal">
                ...
                <div class="row">
                    <div class="form-group col-md-5 ">
                        ...
                        {{ html()->select('facturado', $facturado,  $actividadGasto->nro_factura!=null ? '1' : '0' )
                    ...
                    <div class="form-group col-md-5 col-md-push-1">
                            ...
                            {{ html()->text('nro_comprobante')
                                ...
                                ->value($actividadGasto->nro_comprobante)
                                ->readonly()
                                ->autofocus() }}
                        ...
                        <div id="div_factura">
                            ...
                            {{ html()->select('nro_factura', $facturas, $actividadGasto->nro_factura)
                ...
                <div class="row">
                    <div class="form-group col-md-5">
                        ...
                        {{ html()->select('cliente_proveedor_id', $proveedores,$actividadGasto->cliente_proveedor_id!=null ? $actividadGasto->cliente_proveedor_id : '0' )
                    ...
                    <div class="form-group col-md-5 col-md-push-1">
                        ...
                        <input class="form-control" type="text" id="rut" name="rut" readonly value="{{$proveedor->rut}}"  readonly>
                    </div>
                ...
                <div class="row">
                    <div class="form-group col-md-5">
                        ...
                        <input class="form-control" type="date" name="fecha" value="{{$actividadGasto->fecha->format('Y-m-d')  }}" readonly>
                    </div>
                    <div class="form-group col-md-5 col-md-push-1">
                        {{ html()->label('Periodo')->for('periodo') }}
                        <input class="form-control" type="month" name="periodo" value="{{ $actividadGasto->periodo  }}" readonly>
                    </div>
                        ...
                        <tbody>
                            <tr>
                                <td>
                                    ...
                                    <input type="text" class="form-control" id="labor" name="labor" readonly></div>
                                </td>
                                <td>
                                    {{ html()->label('Neto')->for('neto') }}
                                    <div class="input-group m-b"><span class="input-group-addon">$</span>
                                        <input type="number" class="form-control" id="neto" name="neto" min="0" step="any" readonly>
                                    </div>
                                        ...
                                        <table id="table_modal" class="table table-striped table-bordered table-hover">
                                            ...
                                            @foreach($actividad->cuarteles as $cuartel)
                                               <tr>
                                                  ...
                                                  <input class="cuartel_tamanno" type="hidden" name="{{ $cuartel->nombre }}" min=0 value="{{ $cuartel->tamanno }}" readonly>
                                                  </td>
                                                  
                                                  <td><input class="cuartel_input" type="number" min=0 maxlength="7"  value=0 name="{{ $cuartel->nombre }}" cuartelId="{{ $cuartel->id }}" readonly ></td>
                                                </tr>
                                            @endforeach
                                        </table>
                                    </div>
                            ...
                            @php $numero_labores=0; @endphp
                            @foreach($actividadGasto->labores as $labor)
                                @php 
                                $lineId=rand(1, 1000000);
                                $numero_labores++; 
                                @endphp  
                            <tr lineid={{$lineId}}>
                                <td> {{$labor->labor }} <input type="hidden" name="numero_labores" value="'numero_labores'"><input type="hidden"  value="{{$labor->labor }}" name="labor_array[]"></td>
        
                                <td>{{ $labor->neto }}<input type="hidden" class="sum_neto"  value="{{ $labor->neto }}" name="neto_array[]"></td>

                                <td class="text-right"> <b>$ </b>{{ $labor->iva }}<input type="hidden" class="sum_iva"  value="{{ $labor->iva }}" name="iva_array[]"></td>

                                <td class="text-right"> <b>$ </b>{{ $labor->total }}<input type="hidden" class="sum_labores" value="{{ $labor->total }}" name="total_tabla_array[]"></td>
         
                                <td>
                                    @foreach($labor->labores_cuarteles as $cuartel)
                                         <strong> Cuartel:  </strong>{{ $cuartel->cuartel->nombre }} <strong>Ha:</strong> {{ $cuartel->hectareas }}<input type="hidden" name="{{$numero_labores}}-{{ $cuartel->cuartel_id }} " value="{{ $cuartel->hectareas }}"><br>
                        ...
                        <table class="table table-striped table-bordered table-hover">
                                <tr>
                                    ...
                                    <td>
                                        <div class=" col-md-10 col-sm-10 col-xs-12">
                                            <div class="input-group m-b"><span class="input-group-addon">$</span> 
                                            <input class="form-control" type="number" name="neto_total" id="neto_total" value="{{  number_format($actividadGasto->neto, 2, '.', '')  }}" readonly></div>
                                        </div>
                                    </td>
                                </tr>
                                <tr>
                                    ...
                                    <td>
                                        <div class="col-md-10 col-sm-10 col-xs-12">
                                            <div class="input-group m-b"><span class="input-group-addon">$</span> 
                                            <input type="number" class="form-control" id="iva_total" name="iva_total" readonly value="{{ number_format($actividadGasto->iva, 2, '.', '') }}">
                                            </div>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="col-md-10 col-sm-10 col-xs-12">
                                            <div class="input-group m-b"><span class="input-group-addon">$</span> 
                                            <input type="number" class="form-control" id="total_labores" name="total_labores" value="{{ number_format($actividadGasto->total, 2, '.', '')  }}" readonly>
                                            </div>
                                        </div>
                                    </td>
                                </tr>
...
//Calcular el total de los campos
 ...
 //Calcular el total de los campos
@if($actividadGasto->nro_comprobante==null)
    $('#nro_comprobante_label').hide();
    $('#nro_comprobante').hide();
@endif

@if($actividadGasto->nro_factura==null)
    $('#div_factura').hide();
@endif
