...
@section('content')
{{ html()->modelForm($factura, 'PATCH', route('admin.facturas.update',$factura))->class('form-horizontal')->attribute('enctype', 'multipart/form-data')->open() }}
     <div>
                    <div class="ibox float-e-margins">
                        <div class="ibox-title">
                            <h5>Editar Factura | Nro. :
                                <b> {{ $factura->numero }}</b>
                            </h5>
                               ...
                               <div class="row">
                                ...
                                <div class="row">
                                    ...
                                    <div class="form-group col-md-5 col-md-push-1">
                                        ...
                                        <input type="date" class="form-control"  id="fecha_entrega" name="fecha_entrega" value="{{ isset($factura->fecha_entrega)?$factura->fecha_entrega->format('Y-m-d'):null }}" required>
                                    </div><!--form-group-->
                                ...
                                <div class="row">
                                    ...
                                    <div class="form-group col-md-5 ">
                                        ...
                                        <input type="text" class="form-control" name="nota_publica" value="{{$factura->nota_publica  }}" maxlength="100">
                                    </div>
                                <div class="form-group col-md-5 col-md-push-1">
                                    {{ html()->label('Nota Privada')->for('nota_privada') }}
                                        <input type="text" class="form-control" name="nota_privada" value="{{$factura->nota_privada  }}" maxlength="100">
                                </div>
                                ...
                                <div class="row">
                                    ...
                                    <div class="form-group col-md-5 col-md-push-1">
                                      {{ html()->label('Estado')->for('estado_factura_id') }}
                                        {{ html()->select('estado_factura_id', $estados,$factura->estado_factura->id)
                                ...
                                <div class="row">
                                    ...
                                    <div class="form-group col-md-5">
                                        {{ html()->label('Fecha Vencimiento')->for('fecha_vencimiento') }}
                                        <input type="date" class="form-control"  id="fecha_vencimiento" name="fecha_vencimiento"  required readonly value="{{ isset($factura->fecha_vencimiento)?$factura->fecha_vencimiento->format('Y-m-d'):null }}">
                                    </div><!--form-group-->
                            ...
                            <table class="table table-striped table-bordered table-hover">
                                <tr>
                                    ...
                                    <td>
                                        <div class=" col-md-10 col-sm-10 col-xs-12">
                                            <div class="input-group m-b"><span class="input-group-addon">$</span> 
                                            <input class="form-control" type="number" name="sub_total" id="sub_total" value="{{number_format($factura->sub_total,2,'.','')}}" readonly></div>
                                        </div>
                                    </td>
                                </tr>
                                <tr>
                                    ...
                                    <td>
                                        <div class=" col-md-10 col-sm-10 col-xs-12">
                                            <input type="number" class="form-control" id="porcentaje_descuento" name="porcentaje_descuento" value="{{number_format( $factura->porcentaje_descuento,2,'.','') }}">
                                        </div>
                                    </td>
                                </tr>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="col-md-10 col-sm-10 col-xs-12">
                                            <div class="input-group m-b"><span class="input-group-addon">$</span> 
                                            <input type="number" class="form-control" id="iva" name="iva" readonly value="{{  number_format( $factura->iva,2,'.','')}}">
                                            </div>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="col-md-10 col-sm-10 col-xs-12">
                                            <div class="input-group m-b"><span class="input-group-addon">$</span> 
                                            <input type="number" class="form-control" id="descuento" name="descuento"  value="{{  number_format( $factura->descuento,2,'.','') }}" readonly>
                                            </div>
                                ...
                                <tr>
                                    <td>
                                        {{ html()->label('Total')->for('total_factura') }}
                                    </td>
                                    <td>
                                        <div class="col-md-10 col-sm-10 col-xs-12">
                                            <div class="input-group m-b"><span class="input-group-addon">$</span> 
                                            <input type="number" class="form-control" id="total_factura" name="total_factura" value="{{  number_format( $factura->total,2,'.','') }}" readonly>
                                            </div>
                                        </div>
                                    </td>
                                </tr>
...
@section('scripts')
<script>
//**********Manejo de las fechas a ajustar seguna la emisión***********/
function calcular_dias(numDias,fechaAct){
    fechaAct.setDate(fechaAct.getDate() + numDias)
    var anno=fechaAct.getFullYear();
    var dia = ("0" + fechaAct.getDate()).slice(-2);
    var mes = ("0" + (fechaAct.getMonth() + 1)).slice(-2);
    var fecha_formateada = anno + '-' + mes + '-' + dia;
       
    console.info(fecha_formateada);
    $( '#fecha_vencimiento' ).val(fecha_formateada);
...
$( '#condicion_pago_id' ).on('change', function() {
    var cond=$('#condicion_pago_id').val();
    var dias=0;
    if (cond>=1 || cond<=3){
        $( '#fecha_vencimiento' ).val($( '#fecha_entrega' ).val());
    }

    var fecha = new Date($('#fecha_entrega').val());
   
    if (cond==4){
        calcular_dias(30,fecha);
        
    }
     if (cond==5){
        calcular_dias(60,fecha);
    }
    if (cond==6){
       calcular_dias(120,fecha);
    }
...
function agregar_productos(){
    ...
    //$('#productos option:selected').text()!='Seleccione Producto' ||
    if(( $('#desc_libre').val() != '' ) && $('#precio_compra').val()!='0'  && $('#cantidad').val()!='0' && $('#precio_compra').val()!=''  && $('#cantidad').val()!='' ){
...
function calcular(){
    ...
    $("#total_factura").val(totPres.toFixed(2));
 }
 ...
 function clean() {
    ...
    $("#entrada_libre").prop('checked', true);
 }
 ...
 $("body").on('click','.borrar_productos', function (e) {
    ...
    $( 'select#productos' ).on( 'change', function() {
        ...
        var id=$(this).val();
        $.ajax({
                'url':"{{route('admin.facturas.getProductos')}}",
                'type': "GET",
                'data':{producto_id:id},
